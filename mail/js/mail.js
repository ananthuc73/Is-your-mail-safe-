...
function getIEVersionOrFalse() {
    var n = window.navigator.userAgent, t = n.indexOf("MSIE "), r = n.indexOf("Trident/"), i;
    return t > 0 ? parseInt(n.substring(t + 5, n.indexOf(".", t)), 10) : r > 0 ? (i = n.indexOf("rv:"),
    parseInt(n.substring(i + 3, n.indexOf(".", i)), 10)) : !1
}
function formatDate(n) {
    var t = new Date(n);
    return t.getDate() + " " + ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][t.getMonth()] + " " + t.getFullYear() + ", " + (t.getHours() < 10 ? "0" : "") + t.getHours() + ":" + (t.getMinutes() < 10 ? "0" : "") + t.getMinutes()
}
...
function search(n) {
    var r = IsEmailValid(n), t = "Pwned " + (r ? "email" : "username"), i;
    $("#loading").fadeIn(200);
    $(".pwnedSearchResult.panel-collapse.in").collapse("hide");
    AccountIsDomainSearch(n) ? showFailure(t, "Domain search", 'Can\'t wildcard search, try <a href="DomainSearch">the domain search feature<\/a> instead') : AccountIsValid(n) ? (i = $(".panel-collapse.in").length === 0,
    setTimeout(function() {
        getPwnage(n, r, t);
        i = !1
    }, i ? 0 : 400)) : showFailure(t, "Invalid address (client)", "That's not a real account!")
}
function getPwnage(n, t, i) {
    ...
    $.get(r).done(function(n) {
        showPwnageDetails(n.Breaches, n.Pastes, t)
    }).fail(function(n) {
        $(".passwordManagerLink").attr("href", "https://1password.com/haveibeenpwned/goodnews/");
        n.status === 404 ? (t ? $("#noPastesFound").show() : $("#noPastesFound").hide(),
        $("#noPwnage").collapse("show"),
        incrementSearchResults(0, 0)) : n.status === 403 ? showFailure(i, "Forbidden", "Your request has been forbidden") : n.status === 429 ? showFailure(i, "Rate limited", "Your request has been rate limited, try again later") : showFailure(i, "Oh no, catastrophic failure!", "Oh no - catastrophic failure!")
    }).always(function() {
        $("#loading").fadeOut(200);
        $("#Account").focus();
...
function showPwnageDetails(n, t, i) {
    var r = "", u;
    n === null ? (r += 'Not pwned on any <a href="/FAQs#DataSource">breached sites<\/a>',
    r += i ? ", but found " : "") : n.length === 1 ? (r += 'Pwned on 1 <a href="/FAQs#DataSource">breached site<\/a>',
    r += i ? " and found " : "") : (r += "Pwned on " + n.length + ' <a href="/FAQs#DataSource">breached sites<\/a>',
    r += i ? " and found " : "");
    i && (r += t === null ? 'no <a href="/FAQs#Pastes">pastes<\/a>' : t.length === 1 ? '1 <a href="/FAQs#Pastes">paste<\/a>' : t.length + ' <a href="/FAQs#Pastes">pastes<\/a>');
    ...
    n !== null && ($("#breachDescription").collapse("show"),
    $.each(n, function(n, t) {
        if ($("#" + t.Name).length === 0) {
    ...
    t !== null && ($("#pasteDescription").collapse("show"),
    u = '<div class="pwnedSearchResult pwnedWebsite panel-collapse collapse in" id="pastes"><div class="container"><div class="row"><table class="table-striped"><thead><tr><th>Paste title<\/th><th>Date<\/th><th class="text-right">Emails<\/th><\/tr><\/thead><tbody>',
    $.each(t, function(n, t) {
        u += "<tr>";
        var i;
        ...
        u += t.Title === null ? '<td class="noPasteTitle"><a href="' + i + '" target="_blank" rel="noopener">No title<\/a><\/td>' : '<td><a href="' + i + '" target="_blank" rel="noopener">' + t.Title + "<\/a><\/td>";
    ...
    u += "<\/tbody><\/table><\/div><\/div><\/div>",
    $("#pastes").append(u),
    $("pastes").collapse("show"));
    $(".passwordManagerLink").attr("href", "https://1password.com/haveibeenpwned/ohno/")
}
function incrementSearchResults(n, t) {
    var r = parseInt(getCookie("Searches")), f = isNaN(r) ? 1 : r + 1, u = parseInt(getCookie("BreachedSites")), i;
    n = isNaN(u) ? n : u + n;
...
$(function() {
    $(".pwnedCompanyList a").click(function(n) {
        ...
        $("#pwnModelHeading").text("Loading..."),
        $("#pwnModalBody").html('<i class="fa fa-3x fa-cog fa-spin fa-loader"><\/i>'),
        $.getJSON("/api/v2/breach/" + t, function(n) {
            showBreach(n)
        }).fail(function() {
...
$(document).ready(function() {
    ...
    $("#notifySubmission #notifyMeForm").submit(function(n) {
        ...
        n.preventDefault();
        t = $("#NotifyEmail").val();
        IsEmailValid(t) ? (i = $(this).serialize(),
        $("#notifyError").hide(),
        $("#notificationLoading").css("display", "inline-block"),
        $.post("/api/notifyme", i).done(function() {
            $("#notifySubmission").collapse("hide");
            setTimeout(function() {
                $("#notifySuccess").collapse("show")
            }, 400)
        }).fail(function(n) {
            var t = $.parseJSON(n.responseText).Message;
            n.status == 400 && t == "NoCaptcha" ? ($("#notifyError").text("You must try to prove you're not a robot!"),
            ga("send", "event", "Subscription", "Signup", 'Missing "No CAPTCHA reCAPTCHA"')) : n.status == 400 && t == "BadCaptcha" ? ($("#notifyError").text("You didn't prove that you're not a robot!"),
            ga("send", "event", "Subscription", "Signup", 'Invalid "No CAPTCHA reCAPTCHA"')) : n.status == 400 && t == "BadEmail" ? ($("#notifyError").text("That's not a real email address!"),
            ga("send", "event", "Subscription", "Signup", "Invalid address (server)")) : ($("#notifyError").text("Uh oh, something went wrong. Try again?"),
            ga("send", "event", "Subscription", "Signup", "Generic failure"));
            $("#notifyError").show()
        }).always(function() {
            $("#notificationLoading").hide();
            hideKeyboard()
        })) : ($("#notifyError").text("That doesn't look like a valid email address"),
        $("#notifyError").show(),
        ga("send", "event", "Subscription", "Signup", "Invalid address (client)"))
    });
